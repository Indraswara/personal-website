...
// Similarity Data
// ColorSimilarity.ts/TextureSimilarity.ts run as separate `ts-node`
// subprocesses and rewrite result.js on disk with fresh output (see
// Saving.ts) — but `require('./result')` above only reads that file once,
// at server startup, and Node caches the module after that. Without
// re-reading here, every visitor would see whatever result.js happened to
// contain when the server booted, no matter what they just uploaded and ran.
app.get('/api/similarityData', (req, res) => {
  delete require.cache[require.resolve('./result')];
  res.json(require('./result'));
});

// Show gallery
app.get('/api/get-image/:filename', (req, res) => {
  // path.basename strips any directory components — without it, a filename
  // like "../../etc/passwd" would resolve outside dataset-uploads/ entirely
  // (path.join happily follows ".." segments), and this endpoint is public.
