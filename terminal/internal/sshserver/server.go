...
func New(cfg Config, reg *registry.Registry, orch *sandbox.Orchestrator) (*ssh.Server, error) {
	menu := menuMiddleware(reg, cfg.SSHHost, orch)
...
func menuMiddleware(reg *registry.Registry, sshHost string, orch *sandbox.Orchestrator) wish.Middleware {
		...
		return func(sess ssh.Session) {
			...
			for {
				...
				case tui.ActionPlayground:
					...
					if err := orch.Attach(sess.Context(), sbxRW, true, pty.Term, resizeCh, remoteIP, sessionID); err != nil {
